...
const ORDER_INGESTION_ROLES: ReadonlySet<string> = new Set([
  'internal:heureka-service:orders',
]);

/**
 * Auth RS256 gate for Heureka order ingestion.
 *
...
export class HeurekaOrderIngestionGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (!roles.some((role) => ORDER_INGESTION_ROLES.has(role))) {
      throw new ForbiddenException('Principal is not permitted to ingest Heureka orders');
    }

    request.user = {
      id: 'service:heureka-order-ingestion',
      roles: roles.filter((role) => ORDER_INGESTION_ROLES.has(role)),
    };
    return true;
  }
  ...
  private async validateRoles(token: string): Promise<string[]> {
    ...
    try {
      ...
      console.error(
        JSON.stringify({
          level: 'error',
          event: 'heureka_order_ingestion_auth_validate_unreachable',
          message: 'Auth validate unreachable during Heureka order ingestion',
