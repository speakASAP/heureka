...
check('probes role-scoped routes, never /health', () => {
  ...
  assert(text.includes('/api/orders/admin/lifecycle'), 'does not probe the orders lifecycle route');
...
check('both principals match auth exactly', () => {
  const text = fs.readFileSync(path.join(root, 'src/health/credential-self-reporter.ts'), 'utf8');
  for (const p of [
    'svc-heureka-service--orders-microservice@internal.alfares.cz',
  ]) {
    assert(text.includes(`'${p}'`), `principal ${p} does not match the address auth lists`);
  }
});
...
check('postbuild copies the vendored module into dist', () => {
  ...
  assert(
    postbuild.includes('health/vendor'),
    'package.json has no postbuild step copying src/health/vendor into dist/',
  );
});
